    ...
    <div className="space-y-4">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span className="flex items-center gap-2">
              ...
              {value.length === 0 ? "انتخاب برچسب‌ها" : `${value.length} برچسب انتخاب شده`}
            </span>
        ...
        <PopoverContent className="w-[300px] p-0" align="start">
          <Command>
            <CommandInput placeholder="جستجوی برچسب‌ها..." className="text-right" />
            <CommandList>
              <CommandEmpty>
                {isLoading ? (
                  ...
                  <p className="py-6 text-center text-sm text-muted-foreground">برچسبی یافت نشد.</p>
                )}
              </CommandEmpty>
              <CommandGroup>
                {tags && tags.map((tag) => (
                  ...
                  >
                    ...
                    {value.includes(tag.id) && (
                      <span className="mr-auto flex h-4 w-4 items-center justify-center">
        ...
        <div className="flex flex-wrap gap-2">
          {selectedTags.map((tag) => (
            ...
            >
              ...
              >
                <X className="h-2 w-2" />
                <span className="sr-only">حذف {tag.name}</span>
              </Button>
            </Badge>
          ))}
        </div>
